...
static void check_asan_opts(void) {
  ...
  if (x) {
    ...
    if (!strstr(x, "abort_on_error=1"))
      printf("Custom ASAN_OPTIONS set without abort_on_error=1 - please fix!\n");

    if (!strstr(x, "symbolize=0"))
      printf("Custom ASAN_OPTIONS set without symbolize=0 - please fix!\n");
...
static void maybe_update_plot_file() {
  ...
  fprintf(plot_file, 
          "%llu, %d, %d\n",
          (get_cur_time() - start_plot_time) / 1000 , total_execs, count_non_255_bytes(virgin_bits) ); /* ignore errors */
...
void check_binary(u8* fname) {
  ...
  u32 fd;
  u8* f_data;
  u32 f_len = findSize(fname);

  printf("Validating target binary... %d\n.", f_len);
  ...
  if ((!strncmp(target_path, "/tmp/", 5) && !strchr(target_path + 5, '/')) ||
     ...
     printf("Please don't keep binaries in /tmp or /var/tmp");

  fd = open(target_path, O_RDONLY);

  if (fd < 0) printf("Unable to open '%s'", target_path);

  f_data = mmap(0, f_len, PROT_READ, MAP_PRIVATE, fd, 0);

  if (f_data == MAP_FAILED) printf("Unable to mmap file '%s'", target_path);
  ...
  if (f_data[0] == '#' && f_data[1] == '!') {
    ...
    printf("Program '%s' is a shell script", target_path);
  ...
  if (f_data[0] != 0x7f || memcmp(f_data + 1, "ELF", 3))
    printf("Program '%s' is not an ELF binary", target_path);

#else

  if (f_data[0] != 0xCF || f_data[1] != 0xFA || f_data[2] != 0xED)
    printf("Program '%s' is not a 64-bit Mach-O binary", target_path);

#endif /* ^!__APPLE__ */

  if (!memmem(f_data, f_len, SHM_ENV_VAR, strlen(SHM_ENV_VAR) + 1)) {

    perror("No instrumentation detected");
  ...
  if (memmem(f_data, f_len, PERSIST_SIG, strlen(PERSIST_SIG) + 1)) {
    ...
    printf("Persistent mode binary detected.");
    setenv(PERSIST_ENV_VAR, "1", 1);

  } else if (getenv("AFL_PERSISTENT")) {

    printf("AFL_PERSISTENT is no longer supported and may misbehave!");

  }

  if (memmem(f_data, f_len, DEFER_SIG, strlen(DEFER_SIG) + 1)) {

    printf("Deferred forkserver binary detected.");
    setenv(DEFER_ENV_VAR, "1", 1);

  } else if (getenv("AFL_DEFER_FORKSRV")) {

    printf("AFL_DEFER_FORKSRV is no longer supported and may misbehave!");

  }

  if (munmap(f_data, f_len)) perror("unmap() failed");
