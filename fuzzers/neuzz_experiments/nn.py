...
def process_data():
    ...
    label_to_index = {}
    for idx, f in enumerate(counter):
        label_to_index[int(f[0])] = idx
    bitmap = np.zeros((len(seed_list), len(label)))
    for idx, i in enumerate(seed_list):
        tmp = raw_bitmap[i]
        for j in tmp:
            if int(j) in label_to_index:
                bitmap[idx][label_to_index[int(j)]] = 1
...
def vectorize_file(fl):
    ...
    if ln < MAX_FILE_SIZE:
        tmp = tmp + (MAX_FILE_SIZE - ln) * b'\x00'
    seed[0] = [j for j in bytearray(tmp)][:MAX_FILE_SIZE]
...
def gen_mutate2(model, edge_num, sign):
    ...
    if(round_cnt == 0):
        ...
        vari_seeds = glob.glob("./vari_seeds/*")
        new_seed_list = new_seeds + vari_seeds 
